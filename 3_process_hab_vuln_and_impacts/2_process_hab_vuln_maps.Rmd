...
Collect the habitat maps, and for each stressor, multiply the habitat proportions by the habitat vulnerability score.
...
Gather habitat maps at 1 km bc, and put into a stack.  Mask to ocean cells only.

```{r gather hab maps}
hab_map_fs <- list.files(here('_data/habitat_maps'), pattern = '.tif', full.names = TRUE)

ocean_a  <- rast(here('_spatial/ocean_bc_1km.tif'))
...
### sum maxes out at 7.82 (?), so yes, sum of habitats can be
...
```{r loop over stressors and calc vuln maps}
prs <- hab_vulns$pressure
habs <- names(hab_maps)

for(p in prs) {
  ### p <- prs[1]
  out_fstem <- here_anx('_output/vuln_maps/vuln_maps_habitat/vuln_hab_%s_mean.tif')
...
map_fstem <- here_anx('_output/vuln_maps/vuln_maps_habitat/vuln_hab_%s_mean.tif')
