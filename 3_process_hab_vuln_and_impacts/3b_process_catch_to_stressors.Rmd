...
Cell values match LOICZID values from AquaMaps.  Apply catch values to HCAF raster; reproject to bc 1 km resolution.  Check for coastal gaps due to mismatch between Watson data and coastline data, and gapfill these to approximate Jamie's gapfilling methods from RPoC.  Note, some missing coastline values will be legit non-fished areas; if we use a focal() style window of one or two cells this shouldn't distort values too much.  Note that Jamie's analysis seems to have gapfilled at a larger resolution (22.5 km x 22.5 km?), based on half-degree cells reprojected without setting resolution; here we reproject directly to the 1 km analysis resolution.

* Project catch to bc 1 km x 1 km
* Gapfill missing cells using `focal()` with a 11x11 window (Jamie's method used a 22.5 km radius; here we'll just do 5 km), then mask this to just coastal cells and add back to the unfilled data.

```{r gapfill function}
gapfill_to_coast <- function(r) {
  coast_bc <- rast(here('_spatial/coastal_3nmi_buffer_bc_1km.tif'))
  ocean_bc <- rast(here('_spatial/ocean_bc_1km.tif'))

  r_gf_coast <- r %>%
    focal(w = 11, fun = mean, na.rm = TRUE, na.policy = 'only') %>%
    mask(coast_bc)
...
for(ct in catch_type_vec) { ### ct <- catch_type_vec[1]
  ...
  hcaf_r <- rast(xmin = -180, xmax = +180, ymin = -90, ymax = +90,
                 resolution = 0.5, crs = '+init=epsg:4326') %>%
    setValues(1:ncell(.))
    
  catch_hcaf <- subst(hcaf_r, 
                      from = catch_type_df$cell_id, 
                      to = catch_type_df$tot_catch_km2, 
                      others = NA)
  
  catch_bc <- project(catch_hcaf, ocean_bc, method = 'near')
  
  catch_bc_gf <- gapfill_to_coast(catch_bc)
  
  writeRaster(catch_bc_gf, outf, overwrite = TRUE)
}
...
# tot_catch_map <- rast(catch_map_fs) * ocean_bc * 100 %>% sum(na.rm = TRUE)
...
* `here_anx('stressors/npp/log_npp_present_surf_gf_bc.tif')`
* `here_anx('stressors/npp/log_npp_present_benth_gf_bc.tif')`

Use surface for pelagics/artisanal and benthic for demersal.

```{r}
pel_log_npp <- rast(here_anx('stressors/npp/log_npp_present_surf_gf_bc.tif'))
dem_log_npp <- rast(here_anx('stressors/npp/log_npp_present_benth_gf_bc.tif'))
