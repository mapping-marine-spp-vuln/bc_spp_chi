...
* `_data/stressors_bc/bycatch_pelagic_2017.tif`
* `_data/stressors_bc/bycatch_benthic_2017.tif`
...
```{r load bycatch stressor layers}
benth_rast <- rast(here('_data/stressors_bc/bycatch_benthic_2017.tif'))
pelag_rast <- rast(here('_data/stressors_bc/bycatch_pelagic_2017.tif'))
...
for(t in taxa) {
  ...
  ### let's break this into chunks by cell_id
  chunk_size <- 500000
  n_cells  <- rast(here('_spatial/ocean_bc_1km.tif')) %>% ncell()
  n_chunks <- ceiling(n_cells / chunk_size)
  n_cores  <- ceiling(16 * 1e6 / nrow(tx_maps))
  # system.time({
  ...
  rast_mean <- map_to_bc(result_df, which = 'impact_mean')
  rast_sd   <- map_to_bc(result_df, which = 'impact_sd')
  rast_nspp <- map_to_bc(result_df, which = 'n_spp')
...
if(!file.exists(impact_f_mean)) {
  ...
  rast_mean <- map_to_bc(spp_mean, which = 'imp_mean')
...
if(!file.exists(impact_f_sdev)) {
  ...
  rast_sdev <- map_to_bc(spp_sdev, which = 'imp_sdev')
...
# spp_nspp <- big_df %>% group_by(cell_id) %>% summarize(nspp = sum(imp_nspp))
# rast_nspp <- map_to_bc(spp_nspp, which = 'nspp')
# main_nspp <- rast(here('_output/nspp_maps/species_richness'))
# x <- as.vector(values(rast_nspp)); y <- as.vector(values(main_nspp))
# match <- (is.na(x) & is.na(y)) | x == y
# all(match)
```

```{r plot spp impact maps}
...
plot(log10(mean_rast_spp), col = map_cols, 
...
tmp_stem_fe <- here_anx('impact_maps_by_taxon/impacts_tx_bycatch',
                        'imp_fvwt_summary_chunk_%s_to_%s_bycatch.csv')
### zxcv <- list.files(dirname(tmp_stem_fe), pattern = 'imp_fvwt_summary_chunk', full.names = TRUE)
### unlink(zxcv)
...
for(chunk_i in 1:n_chunks) { 
  ...
  tmp_bycatch_fe <- sprintf(tmp_stem_fe, chunk_start, chunk_end)
  if(file.exists(tmp_bycatch_fe)) {
  ...
  if(nrow(chunk_maps_raw) == 0) {
    ...
    write_csv(data.frame(), tmp_bycatch_fe)
    next()
  }
  ...
  write_csv(chunk_str_sum, tmp_bycatch_fe)
}
```
...
rast_fe_out_stem <- here_anx('_output/impact_maps/impact_maps_funct_entity', 
                         'impact_fe_bycatch_%s.tif')

rast_mean_imp_fe_f <- sprintf(rast_fe_out_stem, 'mean')
rast_sdev_imp_fe_f <- sprintf(rast_fe_out_stem, 'sdev')

if(any(!file.exists(rast_mean_imp_fe_f, rast_sdev_imp_fe_f))) {
  ...
  mean_rast_fe <- map_to_bc(impact_map_df, by = 'cell_id', which = 'fv_wt_mean_impact')
  sdev_rast_fe <- map_to_bc(impact_map_df, by = 'cell_id', which = 'fv_wt_sd_impact')
  
  message('Writing impact rasters for bycatch...')
  writeRaster(mean_rast_fe, rast_mean_imp_fe_f, overwrite = TRUE)
  writeRaster(sdev_rast_fe, rast_sdev_imp_fe_f, overwrite = TRUE)
  
  ### check that n_fe adds up correctly
  # n_fe <- impact_map_df %>% data.table() %>% .[ , n_fe := sum(n_fe), by = 'cell_id']
  # rast_n_fe <- map_to_bc(n_fe, which = 'n_fe')
  # main_n_fe <- rast(here('_output/nspp_maps/funct_entity_richness'))
...
```{r plot fv-weighted impact maps}
mean_rast_fe <- rast(rast_mean_imp_fe_f)
sdev_rast_fe <- rast(rast_sdev_imp_fe_f)
cv_rast_fe <- sdev_rast_fe / mean_rast_fe

map_cols <- hcl.colors(n = 50)

plot(log10(mean_rast_fe), col = map_cols, 
