...
catch_mapfile_df <- data.frame(f = list.files(dirname(catch_map_stem), full.names = TRUE)) %>%
  mutate(src = str_remove(basename(f), '_.+')) %>%
  mutate(species = str_remove_all(basename(f), '.+rescaled_catch_|.csv'),
         species = str_replace_all(species, '_', ' ')) %>%
  rename(catch_map_f = f)
...
for(t in taxa) {
  ...
  message('Taxon ', t, ' range map dataframe: ', nrow(tx_range_maps), 
          ' range observations for ', nrow(tx_range_mapfiles), ' species...')
  ...
  chunk_size <- 500000
  base_r   <- rast(here('_spatial/ocean_bc_1km.tif'))
  n_cells  <- ncell(base_r)
  n_chunks <- ceiling(n_cells/chunk_size)
  n_cores <- ceiling(6 * 1e6 / nrow(tx_catch_range_maps))
...
```{r moar helper fxns}
combine_taxa_maps <- function(tx_bio_rem_map_df) {
  
  mean_fs <- tx_bio_rem_map_df %>% filter(p == 'mean') %>% .$f
  sdev_fs <- tx_bio_rem_map_df %>% filter(p == 'sdev') %>% .$f
  nspp_fs <- tx_bio_rem_map_df %>% filter(p == 'nspp') %>% .$f
  taxa <- tx_bio_rem_map_df$t %>% unique()
...
if(any(!file.exists(impact_f_mean, impact_f_sdev))) {
  ...
  # rast_nspp_catch <- map_to_bc(catch_spp_nspp, which = 'imp_nspp')
...
if(!file.exists(impact_f_mean)) {
  ...
  rast_mean <- map_to_bc(catch_spp_mean, which = 'imp_mean')
...
if(!file.exists(impact_f_sdev)) {
  ...
  rast_sdev <- map_to_bc(catch_spp_sdev, which = 'imp_sdev')
...
read_truncated_rangemap <- function(f, chunk_start, chunk_end) {
  if(file.exists(f)) {
    ...
    df <- data.frame(cell_id = -1, map_f = f)
  }  
  if(nrow(df) == 0) {
    return(data.frame(cell_id = -1, map_f = f)) 
  } else {
    return(df)
  }
}
...
calc_chunk_str_sum <- function(chunk_maps, spp_for_calc) {
  ...
  chunk_impact_list <- parallel::mclapply(cell_gps, mc.cores = 8,
...
for(chunk_i in 1:n_chunks) { 
  ...
  chunk_rangemaps_list <- parallel::mclapply(range_mapfile_df$range_map_f, mc.cores = 12, 
  ...
  chunk_rangemaps_raw <- bind_maps_list(chunk_maps_list = chunk_rangemaps_list, 
    ...
    filter(cell_id > 0)
  if(nrow(chunk_rangemaps_raw) == 0) {
    ### no spp - write out empty file and move on
    message('No species found for ', chunk_text,
            '... writing empty file...')
  ...
  message('... Loading ', nrow(catch_mapfile_df), ' catch maps cropped for ', 
          chunk_text,  '...')
  chunk_catch_maps_list <- parallel::mclapply(catch_mapfile_df$catch_map_f, mc.cores = 12, 
...
if(any(!file.exists(rast_mean_impact_fe_f, rast_sdev_impact_fe_f))) {
  ...
  impact_map_df <- parallel::mclapply(chunk_files, mc.cores = 12, 
  ...
  # n_fe_rast_fvwt <- map_to_bc(impact_map_df, by = 'cell_id', which = 'n_fe')
