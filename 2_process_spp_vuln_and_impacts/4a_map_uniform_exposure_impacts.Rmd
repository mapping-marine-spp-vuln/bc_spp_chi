...
Note: because there are no benthic structures in our data for BC, we will manually remove it here.
...
str_vuln_lookup <- str_vuln_lookup_raw %>%
  filter(!stressor %in% c('biomass_removal', 'bycatch', 'sst_rise', 'benth_str'))
```
...
```{r}
str_fs <- list.files(here('_data/stressors_bc'), full.names = TRUE)
...
vuln1_mean_fs <- list.files(here_anx('_output/vuln_maps/vuln_maps_by_species'), 
                            pattern = 'mean.tif', full.names = TRUE)
vuln1_mean_fs <- vuln1_mean_fs[str_detect(vuln1_mean_fs, vuln_string)]

vuln1_mean_stack <- terra::rast(vuln1_mean_fs) %>%
  setNames(basename(vuln1_mean_fs) %>% str_remove_all('vuln_spp_|_mean.tif'))

vuln1_sd_fs <- list.files(here_anx('_output/vuln_maps/vuln_maps_by_species'), 
                          pattern = 'sdev.tif', full.names = TRUE)
vuln1_sd_fs <- vuln1_sd_fs[str_detect(vuln1_sd_fs, vuln_string)]

vuln1_sd_stack <- terra::rast(vuln1_sd_fs) %>%
  setNames(basename(vuln1_sd_fs) %>% str_remove_all('vuln_spp_|_sdev.tif'))
```
...
for(i in 1:nrow(str_vuln_lookup)) {
  ...
  f_stem <- here_anx('_output/impact_maps/impact_maps_species', 
                 'impact_spp_%s_x_%s_%s.tif')
...
imp_map_dir <- here_anx('_output/impact_maps/impact_maps_species')
imp_map_fs <- list.files(imp_map_dir, pattern = '_mean.tif', full.names = TRUE)

focal_strs <- str_vuln_lookup$stressor %>% unique()
...
```{r}
vuln2_mean_fs <- list.files(here_anx('_output/vuln_maps/vuln_maps_by_funct_entity'), 
                       pattern = 'mean.tif', full.names = TRUE)
vuln2_mean_fs <- vuln2_mean_fs[str_detect(vuln2_mean_fs, vuln_string)]

vuln2_mean_stack <- terra::rast(vuln2_mean_fs) %>%
  setNames(basename(vuln2_mean_fs) %>% str_remove_all('vuln_fe_|_mean.tif'))

vuln2_sd_fs <- list.files(here_anx('_output/vuln_maps/vuln_maps_by_funct_entity'), 
                       pattern = 'sdev.tif', full.names = TRUE)
vuln2_sd_fs <- vuln2_sd_fs[str_detect(vuln2_sd_fs, vuln_string)]

vuln2_sd_stack <- terra::rast(vuln2_sd_fs) %>%
  setNames(basename(vuln2_sd_fs) %>% str_remove_all('vuln_fe_|_sdev.tif'))
```
...
for(i in 1:nrow(str_vuln_lookup)) {
  ...
  f_stem <- here_anx('_output/impact_maps/impact_maps_funct_entity', 
                 'impact_fe_%s_x_%s_%s.tif')
  f_out  <- sprintf(f_stem, s, v, c('mean', 'sdev'))
...
imp_map_dir <- here_anx('_output/impact_maps/impact_maps_funct_entity')
...
mean_unwt_fs <- list.files(here_anx('_output/impact_maps/impact_maps_species'), 
                           pattern = '_mean.tif', full.names = TRUE)
mean_fvwt_fs <- list.files(here_anx('_output/impact_maps/impact_maps_funct_entity'), 
                           pattern = '_mean.tif', full.names = TRUE)

ocean_mask <- terra::rast(here('_spatial/ocean_bc_1km.tif'))
...
for(f in focal_strs) { ### f <- focal_strs[5]
  ...
  if(length(unwt_f) > 1) {
    unwt_r <- terra::rast(unwt_f) %>% 
      sum(na.rm = TRUE) %>%
      mask(ocean_mask)
    fvwt_r <- terra::rast(fvwt_f) %>% 
      sum(na.rm = TRUE) %>%
      mask(ocean_mask)
  } else {
    unwt_r <- terra::rast(unwt_f)
  ...
  plot(mean_diff_squished$r, col = map_cols, 
       main = paste0('% Diff in mean impact: ', str_name, '...'),
       zlim = c(-mean_diff_squished$zlim, mean_diff_squished$zlim), 
       legend = TRUE, axes = FALSE)  
  plot(m_d_mask_0.10, col = map_cols, 
       main = paste0('% Diff in mean impact masked 0.10: ', str_name, '...'),
       zlim = c(-mean_diff_squished$zlim, mean_diff_squished$zlim), 
       legend = TRUE, axes = FALSE)  
  plot(m_d_mask_0.25, col = map_cols, 
       main = paste0('% Diff in mean impact masked 0.25: ', str_name, '...'),
       zlim = c(-mean_diff_squished$zlim, mean_diff_squished$zlim), 
       legend = TRUE, axes = FALSE)
}
```
