...
Read in vulnerability/stressor lookup, and disaggregate bycatch into benthic and pelagic.  Manually remove benthic structure stressor - none in our dataset.

```{r}
str_vuln_lookup_f <- here('_raw/stressor_vulnerability_lookup.csv')

str_vuln_lookup <- read_csv(str_vuln_lookup_f) %>%
  filter(stressor != 'benth_str') %>%
...
str_fs <- list.files(here('_data/stressors_bc'), pattern = '.tif', full.names = TRUE)
...
get_spp_range <- function(spp_id, src, thresh = .50) {
  ...
  spp_range_f <- here_anx('spp_maps_bc', sprintf('%s_spp_bc_%s.csv', src_fix, spp_id_fix))
...
surface_npp <- rast(here_anx('stressors/npp', 'log_npp_present_surf_gf_bc.tif'))
benthic_npp <- rast(here_anx('stressors/npp', 'log_npp_present_benth_gf_bc.tif'))
...
spp_pelagic <- spp_info_all %>%
  filter(str_detect(wcol, 'pel')) %>%
  .$species %>% unique()
...
summarize_spp_impact <- function(spp_str_intsx, area_df, spp_name) {
  ...
  spp_intsx_area_df <- spp_str_intsx %>%
    ...
    mutate(intensity   = intensity * ocean_a_km2, ### "intensity" in stressor per 100 km^2
           int_npp     = intensity * npp, 
           int_ln_npp  = intensity * log(npp),
  ...
  imp_by_str <- spp_intsx_area_df %>%
    ...
    summarize(### Intensity:
              ...
              int_npp_tot    = sum(int_npp), 
              int_npp_cv     = sd(int_npp) / mean(int_npp),
              int_ln_npp_tot = sum(int_ln_npp), 
              int_ln_npp_cv  = sd(int_ln_npp) / mean(int_ln_npp),
              ### Impacts:
...
coastal_a_rast <- rast(here('_spatial/bathy_bc_neritic.tif'))
  ...
  for(k in 1:n_chunks) {
    ...
    tmp <- parallel::mclapply(seq_along(chunk_spp_vec),
        ...
        spp_str_intsx <- process_spp_str_intsx(tmp_vuln_df = spp_am_vuln, 
          ...
          oharac::dt_join(spp_am_vuln %>% 
                            select(vulnerability, stressor, stressor_group, v_score), 
