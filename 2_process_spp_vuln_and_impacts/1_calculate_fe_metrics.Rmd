...
Here we use the IUCN species range maps rasterized to 10 km Mollweide, and AquaMaps species distributions reprojected to 1 km bc projection.  Priority: IUCN species maps, then AquaMaps with occur_cells ≥ 10.  These map filenames are already set up in the `get_spp_info()` function.
...
if(!file.exists(fe_summary_file)) {
  ...
  tmp_chunk_filestem <- here('tmp/fe_summary_chunks/fe_summary_chunk_%s.csv')
  for(i in 1:n_chunks) { ### i <- 1
    ...
    chunk_fe_sumlist <- parallel::mclapply(fe_vec, mc.cores = 8,
  ...
  fe_summary_list <- parallel::mclapply(fe_tmp_fs, FUN = fread, mc.cores = 6)
...
```{r calc n_spp}
n_spp_f <- here('_output/func_entities/n_spp.tif')
n_fe_f  <- here('_output/func_entities/n_fe.tif')

### unlink(n_spp_f); unlink(n_fe_f)
if(!file.exists(n_spp_f) | !file.exists(n_fe_f)) {
  # system.time({
  fe_sum_total <- data.table::fread(fe_summary_file)

  ### Get number of cells from the raster
  chunk_size <- 200000
  n_cells  <- ncell(rast(here('_spatial/ocean_bc_1km.tif')))
  n_chunks <- ceiling(n_cells / chunk_size)

  message('Calculating species richness map...')
  nspp_list <- parallel::mclapply(1:n_chunks, mc.cores = 4,
                 FUN = function(n) { ### n <- 20
  ...
  message('Converting n_spp and n_fe data.frame to rasters...')
  
  n_spp_rast   <- map_to_bc(nspp_df, which = 'n_spp')
  n_fe_rast    <- map_to_bc(nspp_df, which = 'n_fe')

  message('Writing n_spp and n_fe rasters...')
...
### calculating functional vulnerability and redundancy

Calculating vulnerability both as the original (sum of FEs with only one member) and weighted (includes half credit for two members, quarter credit for three, etc).

```{r calculating functional vulnerability metric}
  
f_vuln_f  <- here('_output/func_entities/f_vuln.tif')

### unlink(f_vuln_f)
if(!file.exists(f_vuln_f)) {
  # system.time({
  if(!exists('fe_sum_total')) fe_sum_total <- data.table::fread(fe_summary_file)
  ...
  n_fe_df <- data.frame(values(n_fe_r)) %>%
    mutate(cell_id = 1:n()) %>%
    filter(!is.na(n_fe))

  message('Calculating functional vulnerability map...')
  fv_list <- parallel::mclapply(1:n_chunks, mc.cores = 4,
                 FUN = function(n) { ### n <- 1
                   ...
                   f_vuln <- fe_sum_total %>%
                     filter(between(cell_id, cell_id_min, cell_id_max)) %>%
                     oharac::dt_join(n_fe_df, by = 'cell_id', type = 'left') %>%
                     data.table() %>%
                     .[ , .(f_vuln  = sum(n_spp_fe == 1) / first(n_fe)),
                     by = cell_id]
                 }) 
  ...
  message('Converting f_vuln data.frame to raster...')
  
  f_vuln_rast  <- map_to_bc(f_vuln_df, which = 'f_vuln')

  message('Writing f_vuln raster...')
  writeRaster(f_vuln_rast,  f_vuln_f, overwrite = TRUE)
}

```

```{r calculating weighted functional vulnerability and redundancy metrics}
  
f_wvuln_f <- here('_output/func_entities/f_wvuln.tif')
f_red_f   <- here('_output/func_entities/f_red.tif')

### unlink(f_vuln_f); unlink(f_wvuln_f)
if(!file.exists(f_wvuln_f) | !file.exists(f_red_f)) {
  # system.time({
  if(!exists('fe_sum_total')) fe_sum_total <- data.table::fread(fe_summary_file)
  ...
  message('Converting f_wvuln and f_red data.frame to rasters...')
  
  f_wvuln_rast <- map_to_bc(f_vuln_df, which = 'f_wvuln')
  f_red_rast   <- map_to_bc(f_vuln_df, which = 'f_red')
  
  message('Writing f_wvuln and f_red rasters...')
  writeRaster(f_wvuln_rast, f_wvuln_f, overwrite = TRUE)
  writeRaster(f_red_rast,   f_red_f, overwrite = TRUE)
}

```

### calculating functional overredundancy

```{r}
f_overred_f <- here('_output/func_entities/f_overred.tif')

### unlink(f_overred_f)
if(!file.exists(f_overred_f)) {
  # system.time({
  if(!exists('fe_sum_total')) fe_sum_total <- data.table::fread(fe_summary_file)
  ...
  f_red_r <- rast(f_red_f) %>%
    setNames('f_red') 
  
  f_red_df <- data.frame(values(f_red_r)) %>%
    mutate(cell_id = 1:n()) %>%
    filter(!is.na(f_red))

  message('Calculating functional overredundancy map...')
  f_overred_list <- parallel::mclapply(1:n_chunks, mc.cores = 4,
              FUN = function(n) { ### n <- 1
                message('... funct overred chunk ', n, ' of ', n_chunks, '...')
                cell_id_min <- (n - 1) * chunk_size + 1
                cell_id_max <- n * chunk_size
                f_overred <- fe_sum_total %>%
                  filter(between(cell_id, cell_id_min, cell_id_max)) %>%
                  oharac::dt_join(f_red_df, by = 'cell_id', type = 'left') %>%
                  data.table() %>%
  ...
  message('Converting f_overred data.frame to raster...')
  f_overred_rast <- map_to_bc(f_overred_df, which = 'f_overred')

  message('Writing f_overred to raster...')
  writeRaster(f_overred_rast, f_overred_f, overwrite = TRUE)
}
...
Load rasters
```{r load rasts}
n_spp_rast     <- rast(n_spp_f)
n_fe_rast      <- rast(n_fe_f)
f_vuln_rast    <- rast(f_vuln_f)
f_wvuln_rast   <- rast(f_wvuln_f)
f_red_rast     <- rast(f_red_f)
f_overred_rast <- rast(f_overred_f)
```
