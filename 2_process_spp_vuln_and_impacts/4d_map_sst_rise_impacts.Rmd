...
for(t in taxa) {
  ...
  n_cores <- ceiling(24 * 1e6 / nrow(tx_maps))
  # system.time({
...
# rast_nspp <- map_to_bc(nspp_df, which = 'nspp')
...
if(!file.exists(impact_f_mean)) {
  ...
  rast_mean <- map_to_bc(all_spp_mean, which = 'imp_mean')
...
if(!file.exists(impact_f_sdev)) {
  ...
  rast_sdev <- map_to_bc(all_spp_sdev, which = 'imp_sdev')
...
### zxcv <- list.files(dirname(tmp_stem_fe), pattern = 'impact_summary_chunk.+sst_rise', full.names = TRUE)
### unlink(zxcv)
...
for(chunk_i in 1:n_chunks) { 
  ...
  if(nrow(chunk_maps_raw) == 0) {
    ### null set of cells - e.g., no cells in BC
    chunk_str_sum <- data.frame()
  } else {
...
if(any(!file.exists(rast_mean_impact_fe_f, rast_sdev_impact_fe_f))) {
  ...
  mean_rast_fe <- map_to_bc(impact_map_df, which = 'fv_wt_mean_impact')
  sdev_rast_fe <- map_to_bc(impact_map_df, which = 'fv_wt_sd_impact')
  ...
  # n_fe_rast_fe <- map_to_bc(impact_map_df, which = 'n_fe')
