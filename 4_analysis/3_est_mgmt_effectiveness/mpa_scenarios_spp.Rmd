...
if(!file.exists(chi_spp_sum_f)) {
  ...
  chi_spp_r <- rast(here('_output/cumulative_impact_maps/chi_species.tif')) %>% 
    setNames('chi_spp')
  seaice_r <- rast(here('_spatial/seaice_mask_0.8.tif'))
...
if(!file.exists(chi_spp_map_f)) {
  ...
  chi_top30_r <- map_to_polar(chi_top30_df, which = 'pct_prot')
  chi_top30_r[is.na(chi_top30_r)] <- 0
  chi_top30_r <- chi_top30_r %>%
    mask(rast(here('_spatial/ocean_polar_1km.tif')))
...
rgn_lbls <- read_csv(here('_spatial/eez_rgns/rgn_names_ohi.csv')) %>%
  select(eez = rgn_id, rgn_label)
eez_prot_sum <- lapply(1:100, summarize_by_eez, df = chi_sum_df) %>%
  bind_rows() %>%
  left_join(rgn_lbls) %>%
  mutate(rgn_label = fct_reorder(rgn_label, chi_mean)) %>%
...
eez_prot_sum2 <- lapply(1:100, summarize_by_eez, df = chi_sum2_df) %>%
  bind_rows() %>%
  left_join(rgn_lbls) %>%
  mutate(rgn_label = factor(rgn_label, 
                            levels = levels(eez_prot_sum$rgn_label))) %>%
  pivot_longer(names_to = 'scenario', values_to = 'chi_mean', starts_with('chi')) %>%
...
ggsave('mpa_scenarios_0_100.png', width = 6, height = 4, dpi = 300)
...
chi_sum_df <- read_csv(chi_spp_sum_f) %>%
  ...
  left_join(rgn_lbls) %>%
  select(rgn_label, mean_chi, mean_bycatch = mean_byc, a_km2 = ncell, pct_reduct) %>%
  arrange(-pct_reduct)
