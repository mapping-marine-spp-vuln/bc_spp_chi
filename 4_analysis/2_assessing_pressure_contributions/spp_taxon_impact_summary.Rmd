...
```{r}
spp_imp_sum_file <- here('4_analysis/1_assessing_pressure_contributions/spp_impact_summary.csv')
...
spp_taxa_sum_file <- here('4_analysis/1_assessing_pressure_contributions/spp_taxa_summary.csv')
...
spp_imp_scores <- read_csv(spp_imp_sum_file) %>%
  ...
  mutate(stressor = str_remove(s_r, 'X.+'),
         stressor = str_replace(stressor, 'biomass_removal', 'targeted_fishing'),
...
spp_imp_strs_clean <- tx_imp_df %>%
  ...
  mutate(str_lbl = case_when(str_detect(stressor, 'bycatch') ~ 'bycatch',
                             stressor == 'benth_str' ~ 'benthic structures',
...
cc_strs <- c('SST extremes', 'Ocean acidification', 'Sea level rise', 
             'UV radiation', 'SST rise')
all_strs <- tx_imp_by_str$str_lbl %>% unique()
...
panel_d <- ggplot(tx_imp_by_str, aes(x = mean_prop, y = taxon)) +
  ...
  geom_col(aes(color = str_lbl), fill = NA,
           width = .7, size = 0.25, show.legend = FALSE) +
...
fig1_file <- here('4_analysis/1_assessing_pressure_contributions', 
                  'spp_taxon_impact_summaries.png')
...
fig1_fourpanel_file <- here('4_analysis/1_assessing_pressure_contributions', 
